function random_model(params, row, d_goals)
    N = length(row.neighs)
    return ones(N) / N
end

function optimal_model(params, row, d_goals)
    N = length(row.neighs)
    ps = zeros(N)
    opt_idx = [d_goals[row.puzzle][s_p] < row.d_goal for s_p in row.neighs]
    ps[opt_idx] .= 1/sum(opt_idx)
    return ps
end

function gamma_only_model(params, row, d_goals)
    γ = params#10 ^ (-params)
    # excl_move_idx = findfirst(x -> x[2][1] == row.prev_move[1][1] && x[1][1] == row.prev_move[2][1], row.all_moves)
    # excl_move_idx = findfirst(x -> x[1][1] == row.prev_move[2][1], row.all_moves)
    # excl_moves = excl_move_idx !== nothing ? [row.all_moves[excl_move_idx]] : a_type[]
    excl_moves = [m for m in row.all_moves if m[1][1] == row.prev_move[2][1]]

    dict = propagate_ps(0.0, row.tree)
    # modulate depths by (1-γ)^d
    new_dict = apply_gamma(dict, γ)
    # turn dict into probability over actions
    ps = process_dict(row.all_moves, new_dict, excl_moves)
    return ps
end

function gamma_0_model(params, row, d_goals)
    λ = params
    excl_moves = [m for m in row.all_moves if m[1][1] == row.prev_move[2][1]]

    dict = propagate_ps(0.0, row.tree)
    # modulate depths by (1-γ)^d
    new_dict = apply_gamma(dict, 0.0)
    # turn dict into probability over actions
    ps = process_dict(row.all_moves, new_dict, excl_moves)
    ps = λ/length(ps) .+ (1-λ) * ps
    return ps
end

function eureka_model(params, row, d_goals)
    d, λ = params
    N = length(row.neighs)
    ps = zeros(N)
    if row.d_goal <= d
        opt_idx = [d_goals[row.puzzle][s_p] < row.d_goal for s_p in row.neighs]
        ps[opt_idx] .= 1/sum(opt_idx)
    else
        ps = ones(N) ./ N
    end
    ps = λ/N .+ (1-λ) * ps
    return ps
end

function opt_rand_model(params, row, d_goals)
    λ = params
    N = length(row.neighs)
    ps = zeros(N)
    opt_idx = [d_goals[row.puzzle][s_p] < row.d_goal for s_p in row.neighs]
    ps[opt_idx] .= 1/sum(opt_idx)
    ps = λ/N .+ (1-λ) * ps
    return ps
end

function hill_climbing_model(params, row, d_goals)
    b0, b1, b2 = params
    f1 = row.features[:, 1]
    f2 = row.features[:, 2]
    ex = exp.((b0 .+ (b1 .* f1) .+ (b2 .* f2)))
    return ex ./ sum(ex)
end

function forward_search(params, row, d_goals, F, state_to_idx)
    log_gamma, k = params
    return p_a([Int(k)], row.neighs, F, state_to_idx)[1]
end